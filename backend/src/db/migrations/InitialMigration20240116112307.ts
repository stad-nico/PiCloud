...
export class Migration20240116112307 extends Migration {
	async up(): Promise<void> {
		...
		// prettier-ignore
		this.addSql('CREATE OR REPLACE FUNCTION GET_DIRECTORY_UUID(path VARCHAR(255)) RETURNS varchar(255) DETERMINISTIC BEGIN DECLARE nextPath VARCHAR(255) DEFAULT TRIM(LEADING "/" FROM path); DECLARE nextUuid VARCHAR(255) DEFAULT (SELECT id FROM directories WHERE directories.name = GET_UPMOST_DIRNAME(nextPath) AND directories.parentId IS NULL); SET nextPath = GET_PATH_AFTER_UPMOST_DIRNAME(nextPath); WHILE nextPath != "" DO SET nextUuid = (SELECT id FROM directories WHERE directories.parentId = nextUuid AND directories.name = GET_UPMOST_DIRNAME(nextPath)); SET nextPath = GET_PATH_AFTER_UPMOST_DIRNAME(nextPath); END WHILE; RETURN nextUuid; END')
		// prettier-ignore
		this.addSql('CREATE OR REPLACE FUNCTION GET_DIRECTORY_PATH(id VARCHAR(255)) RETURNS varchar(255) DETERMINISTIC BEGIN DECLARE path VARCHAR(255) DEFAULT (SELECT name FROM directories WHERE directories.id = id); DECLARE nextUuid VARCHAR(255) DEFAULT (SELECT parentId FROM directories WHERE directories.id = id); IF id IS NULL THEN RETURN "/"; END IF; WHILE nextUuid IS NOT NULL DO SET path = CONCAT((SELECT name FROM directories WHERE directories.id = nextUuid), "/", path); SET nextUuid = (SELECT parentId FROM directories WHERE directories.id = nextUuid); END WHILE; RETURN path; END')
		// prettier-ignore
		this.addSql('CREATE OR REPLACE FUNCTION GET_DIRECTORY_SIZE(id VARCHAR(255)) RETURNS bigint(20) DETERMINISTIC BEGIN RETURN (SELECT COALESCE(SUM(size),0) AS size FROM tree INNER JOIN files ON tree.childId = files.id WHERE tree.parentId = id AND files.isRecycled = false); END');
