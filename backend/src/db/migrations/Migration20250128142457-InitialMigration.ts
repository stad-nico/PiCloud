...
export class Migration20250128142457 extends Migration {
	async up(): Promise<void> {
		...
		this.addSql(
			"create table `directories` (`id` varchar(36) not null default UUID(), `name` varchar(255) not null, `parentId` varchar(36) null default 'root', `createdAt` datetime not null default current_timestamp(), `updatedAt` datetime not null default current_timestamp() on update current_timestamp(), `userId` varchar(36) not null, primary key (`id`)) default character set utf8mb4 engine = InnoDB;"
		);
		...
		this.addSql(
			'alter table `directories` add constraint `directories_userId_foreign` foreign key (`userId`) references `users` (`id`) on update cascade;'
		);
		...
		this.addSql(
			'alter table `files` add constraint `files_userId_foreign` foreign key (`userId`) references `users` (`id`) on update cascade;'
		);
	}
